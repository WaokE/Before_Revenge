...
export default function convertCommand(commands) {
    ...
    commandsArray.forEach((command) => {
        ...
        // 레버 입력이 있다면, 처리
        const lever = [];
        while (inputs[0] === "U" || inputs[0] === "D" || inputs[0] === "F" || inputs[0] === "B") {
            lever.push(inputs.shift());
        }
        if (inputs[0] === "+") inputs.shift();
        if (lever.length > 0) result.push(commandImagePath[lever.join("")]);

        // 나머지 버튼 입력 처리
        if (inputs.length > 0) result.push(commandImagePath[inputs.join("")]);
    });

    return (
        <View style={{ flexDirection: "row" }}>
            {result.map((command, index) => {
                return <Image key={index} source={command} style={{ width: 20, height: 20 }} />;
            })}
        </View>
    );
}
